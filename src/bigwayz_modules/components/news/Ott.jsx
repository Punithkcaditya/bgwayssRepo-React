...
// api services
import { _logout, _getHistory, _ottData } from "../../config/api/Api";
// loader
...
class News extends Component {
    ...
    getNews = () => {
        try {
            _ottData()
                .then(resp => {                    
                    if (resp.success) {
                        this.setState({ news: resp.data, newsLoading: false })
                    }
                })
    ...
    render() {
        return (
            <>
                {this.props.checkLoggedIn.isLoading && <Loader />}
                                            ...
                                            this.state.news && this.state.news.map((item, index) => {
                                                return <div className="news-card" key = {index}>
                                                    <div className="news-card-img">
                                                        <img src={item.description.display_image} className="img-responsive" />
                                                    </div>
                                                    <div className="news-text">
                                                        <h5>{item.description.ott_text}</h5>
                                                        <p>{item.description.ott_short_desc.length <= 80 ? item.description.ott_short_desc.slice(0,80) : item.description.ott_short_desc.slice(0,80) + '...' }</p>
                                                        <div>
                                                        <Link to={`ott/${item.description.id}`} className="btn-one btn btn-primary">Read More</Link>
                                                        </div>
                                                    </div>
                                                </div>
                                            })
