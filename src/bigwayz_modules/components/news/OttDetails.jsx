...
// api services
import { _ottDetails } from "../../config/api/Api";
// loader
...
class NewsDetails extends Component {
    ...
    getNews = () => {
        try {
            let req = {
                ott_id: this.props.match.params.id
            }
            _ottDetails(req)
                .then(resp => {
                    if (resp.success) {
                        ...
                        <div className="row justify-content-md-end page-content-wraper">
                            <div className="col-lg-11 col-offset-1">
                                <div className="breadcrumbs">
                                    <ul>
                                        ...
                                        <li><Link to="/ott">Ott</Link> <i className="fas fa-caret-right"></i></li>
                                        <li><Link to="/dashboard">{this.state.news && this.state.news.description.ott_text}</Link></li>
                                    </ul>
                                </div>
                                ...
                                {this.state.news && this.state.news.description && this.state.news.description.ott_stream && 
                                (
                                <div className="news-video-youtube">
                                <iframe src={this.state.news.description.ott_stream}
                            ...
                            <div className="col-lg-11 col-offset-1">
                                <div className="news-detail-page-title">
                                    <h4>{this.state.news && this.state.news.description.ott_short_desc}</h4>
                                    <span>{new Date(this.state.news && this.state.news.ott_date).toLocaleString()}</span>
                                </div>
                                <div className="news-detail-text">
                                    <article>
                                        <p>{this.state.news && this.state.news.description.ott_long_desc}</p>
